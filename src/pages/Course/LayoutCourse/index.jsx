...
import {
  SIDEBAR_COURSE_TEACHER,
  SIDEBAR_COURSE_STUDENT,
...
const LayoutCourse = () => {
  ...
  const [sidebarCourse, setSidebarCourse] = useState(
    (role && role === "teacher" && SIDEBAR_COURSE_TEACHER) ||
      SIDEBAR_COURSE_STUDENT
  );
  const tabId = sidebarCourse.find((item) =>
    checkSelectedTab(item, location.pathname)
  );
  ...
  useEffect(() => {
    if (role === "teacher") setSidebarCourse(SIDEBAR_COURSE_TEACHER);
    else {
  ...
  const DashboardBreadcrumbs = () => {
    ...
    return (
      ...
      >
        ...
        <Link key="4" to={`/course/${courseId}/${tabLink}`}>
          <Typography>
            {tab?.text || ""}
          </Typography>
        </Link>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <ShareTray
                    url={`https://oes.vercel.app/enroll/${courseId}`}
                ...
                <Box>
                  {role === "student" && (
                    <LinearProgressWithLabel
                      value={Math.floor(course?.avg * 100 || 0)}
                    />
                  )}
                </Box>
              </Stack>
